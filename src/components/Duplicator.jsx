...
function Duplicator() {
  ...
  const handleUpload = async () => {
    ...
    reader.onload = async (event) => {
      ...
      console.log("faisal",fileContent);

      const rows = fileContent.split(/\r?\n/);
      console.log("saibaz",rows);
  ...
  const fetchJobBatches = async () => {
    try {
      ...
      const response = await fetch("https://mellow-sun-ckbco7ysvyhi.on-vapor.com/api/job-batch", {
        method: "POST", // Changed to POST to fix 405 error
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({}),
  ...
  return (
    <div className="App">
      {/* <button onClick={()=>{ window.location.reload();}} style={{ position: "fixed", top: "20px", left: "20px", cursor: "pointer"}} >
        Reload
        </button> */}
        <div style={{ padding: "50px", borderRadius: "10px",border:"solid"}}>
      <h1 >Duplicator App</h1>
      <input type="file" accept=".csv" onChange={handleFileChange}  style={{ marginBottom: "20px", fontSize: "20px",padding:"30px",marginLeft:"60px"}}/>
      <div>
      <button onClick={handleUpload} style={{ padding:"15px", borderRadius: "10px",border:"solid",width:"170px",backgroundColor:"white",marginTop:"40px"}} >Upload</button>
      </div>
      </div>
      {isLoading && (
      ...
      {uniqueNumbers.length > 0 && (
        <>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", margin: "20px 0",border:"solid"}}>
            <thead>
              <tr>
                <th>Phone Numbers</th>
              </tr>
...
<button
      onClick={handleClick}
      style={{ position: "fixed", bottom: "20px", right: "20px", cursor: "pointer", padding: "10px", borderRadius: "5px" }}
    >
      Admin Menu
    </button>
      ...
      {showPinPopup && (
        <div className="lightbox">
          <div className="lightbox-content">
            ...
            <button onClick={() => setShowPinPopup(false)}>Cancel</button>
          </div>
        </div>
      )}
      {jobBatches.length > 0 && (
            ...
            <tbody>
              {jobBatches.map((job) => (
                <tr key={job.job_dt}>
                  ...
                  <td>{job.total}</td>
                  <td>
                    <button onClick={() => handleDataDownload(job.job_dt)}>Download</button>
